...
class LocationsPlugin extends GoldPlugin
{
	...
	function store_locator_shortcode($atts, $content = '')
	{
		...
		if (isset($_REQUEST['search_locations']) && isset($_REQUEST['your_location']) && strlen(trim($_REQUEST['your_location'])) > 0)
		{
			...
			$nearest_locations = $this->find_nearest_locations($your_location, $radius_miles, $origin); // second param is radius
...
<?php		
	...
	private function import_locations_from_csv($locations_file){	
		...
		foreach($locations as $location){		
			...
			if(empty($postslist)){				
				...
				//$this->geocode_post_on_save($new_id, $post);
	...
	private function csv_importer(){
		...
		if(empty($_FILES)){		
			echo "<p>Use the below form to upload your CSV file for importing.  <strong>Please Note:</strong> the Google Geocoder API is limited to 2.5k free requests per day.</p>";
			...
			if ( is_wp_error( $result ) ){
				...
				echo "<p>Locations succesfully imported!</p>";
			}
		}
	}
